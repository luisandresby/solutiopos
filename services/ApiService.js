...
const ApiService = axios.create({
  baseURL: "http://pos-nexo-back.test", //window.location.protocol+'//'+window.location.hostname, // import.meta.env.VITE_API_URL,
  headers: {
...
ApiService.clearLocale = function () {
  delete this.defaults.headers.common["current-locale"];
};

ApiService.setLocale = function (locale) {
  this.defaults.headers.common["current-locale"] = locale;
};
    ...
    .map(
      (key) =>
        `${key}=${encodeURIComponent(
          _.isObject(data[key]) ? JSON.stringify(data[key]) : data[key]
        )}`
    )
    .join("&");
};
